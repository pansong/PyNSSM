...
# Evaluate all saved checkpoints (plots metric trends over epochs)
python -m src.eval
```
...
- **`src/eval.py`** -- Evaluates all saved checkpoints, runs inference on each, plots scaled MSE/MAE trends over epochs to `eval_checkpoints.png`
...
**Normalization:** Min-max normalization to [-1, 1]: `normalized = 2 * (raw - min) / (max - min) - 1`. Stats (U_min, U_max, Y_min, Y_max) computed from training data via `compute_normalization_stats()`. Denormalization: `raw = (normalized + 1) / 2 * (max - min) + min`. For inference matrixData, X=[cols 5,6] uses `Y_min[0:2]/Y_max[0:2]`; Y=[cols 3,4] uses `Y_min[2:4]/Y_max[2:4]`.
